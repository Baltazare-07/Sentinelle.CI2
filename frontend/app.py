...
# PAGE TABLEAU DE BORD MAIRIE
    ...
    # ========== CARTE COMMUNALE (EN PREMIER) ==========
    st.markdown("### 🗺️ CARTE COMMUNALE DES SIGNALEMENTS")
    st.markdown("Visualisation de tous les signalements sur la commune")
    
    # Créer une carte plus grande pour la mairie
    mairie_map = folium.Map(location=[5.3415, -4.0142], zoom_start=12, control_scale=True)
    colors = {'en_attente': 'red', 'en_cours': 'orange', 'resolu': 'green'}
    
    for s in st.session_state.signalements:
        color = colors.get(s['statut'], 'gray')
        
        # Popup détaillé pour la mairie
        if s.get('tx_hash') and s['tx_hash'].startswith('0x'):
            etherscan_url = f"https://sepolia.etherscan.io/tx/{s['tx_hash']}"
            short_id = s['id'][:20] + '...' if len(s['id']) > 20 else s['id']
            popup_html = f"""
            <div style="font-family: sans-serif; min-width: 250px;">
                ...
                📍 {s['quartier']}<br>
                🆔 {short_id}<br>
                📅 {s['date'].strftime('%d/%m/%Y') if isinstance(s['date'], datetime.datetime) else s['date'][:10]}<br>
                <span style="color: {color}; font-weight: bold;">{s['statut'].upper()}</span><br>
                🔗 <a href='{etherscan_url}' target='_blank'>Voir sur Etherscan</a>
            </div>
            """
        else:
            ...
            <div style="font-family: sans-serif; min-width: 200px;">
                ...
                📍 {s['quartier']}<br>
                📅 {s['date'].strftime('%d/%m/%Y') if isinstance(s['date'], datetime.datetime) else s['date'][:10]}<br>
                <span style="color: {color}; font-weight: bold;">{s['statut'].upper()}</span>
            </div>
            """
    ...
    with col_right:
        ...
        if total > 0:
            ...
            <div style="background: #f8f9fa; padding: 15px; border-radius: 10px;">
                <p><strong>Total des signalements : {total}</strong></p>
                <div style="margin: 10px 0;">
                    <div style="background: #dc3545; height: 25px; border-radius: 5px; width: {en_attente/total*100}%; display: inline-block;"></div>
                    <span>🔴 En attente: {en_attente} ({en_attente/total*100:.1f}%)</span>
                </div>
                <div style="margin: 10px 0;">
                    <div style="background: #fd7e14; height: 25px; border-radius: 5px; width: {en_cours/total*100}%; display: inline-block;"></div>
                    <span>🟠 En cours: {en_cours} ({en_cours/total*100:.1f}%)</span>
                </div>
                <div style="margin: 10px 0;">
                    <div style="background: #28a745; height: 25px; border-radius: 5px; width: {resolus/total*100}%; display: inline-block;"></div>
                    <span>🟢 Résolus: {resolus} ({resolus/total*100:.1f}%)</span>
                </div>
            </div>
            """, unsafe_allow_html=True)
            
            # Barre de progression
            st.progress(en_attente/total, text="Taux de signalements traités")
    ...
    if st.session_state.show_prise_en_charge and st.session_state.get('selected_signalement'):
        ...
        with col_left2:
            st.info(f"""
            **Signalement #{s['id'][:16]}...**  
            📍 **{s['type']}** - Quartier: {s['quartier']}  
            📅 Signalé le {date_str}  
            📍 Position: {s['lat']:.4f}, {s['lng']:.4f}
            """)
        
        with col_right2:
            agent = st.selectbox("**Assigner à**", ["Koffi A.", "Diallo M.", "Kouadio L.", "Yao B."])
            commentaire = st.text_area("**Commentaire public**", placeholder="Ex: Intervention programmée, équipe dépêchée...")
            date_intervention = st.date_input("**Date intervention prévue**", datetime.datetime.now() + datetime.timedelta(days=7))
            
            col_btn1, col_btn2 = st.columns(2)
            with col_btn1:
                if st.button("✅ VALIDER LA PRISE EN CHARGE", use_container_width=True):
