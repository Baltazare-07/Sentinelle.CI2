...
# Fonction pour charger les signalements depuis le backend
def load_signalements_from_backend():
    try:
        response = requests.get('http://localhost:3001/api/signalements', timeout=5)
...
# PAGE NOUVEAU SIGNALEMENT
    ...
    # TYPE DE PROBLÈME (hors formulaire)
    ...
    # Formulaire
    with st.form(key="signalement_form"):
        # PHOTO
                ...
                try:
                    ...
                    with st.spinner("⏳ Enregistrement sur la blockchain en cours..."):
                         ...
                         if response.status_code in [200, 201]:
                            ...
                            new_id = backend_response.get('id')
                            tx_hash = backend_response.get('tx_hash')
                            blockchain_url = backend_response.get('blockchain_url')
                            ...
                            if blockchain_url:
                                st.markdown(f"🔗 [Voir la transaction sur Etherscan]({blockchain_url})")
                            ...
                            st.error("❌ Impossible de se connecter au backend. Vérifiez que 'npm start' est bien lancé sur le port 3001")
                            st.info("💡 Pour démarrer le backend: cd ../backend && npm start")
                except Exception as e:
                            st.error(f"❌ Erreur lors de l'envoi: {str(e)}")
                    # Initialisation des signalements
                            if 'signalements' not in st.session_state:
                                backend_data = load_signalements_from_backend()
                                if backend_data:
                                    st.session_state.signalements = backend_data
                            else:
                                       # Fallback aux données locales
                                    st.session_state.signalements = [
                                    {'id': 'SIG-001', 'type': 'Route', 'quartier': 'Azito', 'lat': 5.3415, 'lng': -4.0142, 
                                    'statut': 'en_attente', 'date': datetime.datetime.now() - datetime.timedelta(days=2)},
                                    {'id': 'SIG-002', 'type': 'Éclairage', 'quartier': 'Maroc', 'lat': 5.3591, 'lng': -4.0195, 
                                     'statut': 'en_cours', 'date': datetime.datetime.now() - datetime.timedelta(days=5)},
                                    {'id': 'SIG-003', 'type': 'Eau', 'quartier': 'Sicogi', 'lat': 5.3856, 'lng': -3.9974, 
                                     'statut': 'resolu', 'date': datetime.datetime.now() - datetime.timedelta(days=10)},
                                    {'id': 'SIG-004', 'type': 'Route', 'quartier': 'Yopougon', 'lat': 5.3225, 'lng': -4.0552, 
                                     'statut': 'en_attente', 'date': datetime.datetime.now() - datetime.timedelta(days=1)},
                                    {'id': 'SIG-005', 'type': 'École', 'quartier': 'Niagon', 'lat': 5.3591, 'lng': -4.0195, 
                                     'statut': 'en_cours', 'date': datetime.datetime.now() - datetime.timedelta(days=3)},
                                    ]
